...
class TfmModel(PreTrainedModel):
    def __init__(self, config, encoder, finetune_config, use_weighted_mean, unproc, layer):
        super().__init__(config)
...
def train_kfold(args):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kfold.split(train_meta_df)):
        ...
        # Create model
        tfm_model = TfmModel(config, model, config_ft, args.use_weighted_mean, args.unproc, None)
...
def main():
    ...
    parser.set_defaults(use_weighted_mean=True)
